...
def generate(model, name='output', num_bars=8):
    ...
    prev_timestep = var(torch.zeros(NUM_NOTES), volatile=True).unsqueeze(0)

    for t in trange(NOTES_PER_BAR * num_bars):
        beat = var(to_torch(compute_beat(t, NOTES_PER_BAR)), volatile=True).unsqueeze(0)
        current_note, states = sample_timestep(model, prev_timestep, beat, states, temperature=temperature)
