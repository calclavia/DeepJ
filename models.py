...
def note_model(time_steps):
    ...
    """
    note_input = Input(shape=(time_steps, NUM_CLASSES), name='rl/note_input')
    beat_input = Input(shape=(time_steps, NOTES_PER_BAR), name='rl/beat_input')
    ...
    model = Model([note_input, beat_input], [policy, value])
...
def supervised_model(time_steps):
    ...
    # Multi-label
    x = Dense(NUM_CLASSES, name='policy')(x)
